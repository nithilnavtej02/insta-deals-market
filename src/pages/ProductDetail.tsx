...
const ProductDetail = () => {
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      const {
        data: existingConv
      } = await supabase.from('conversations').select('id').or(`and(participant_1.eq.${userProfile.id},participant_2.eq.${product.seller_id}),and(participant_1.eq.${product.seller_id},participant_2.eq.${userProfile.id})`).single();
      if (existingConv) {
  ...
  return <div className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-7xl mx-auto lg:grid lg:grid-cols-2 lg:gap-8 lg:p-8">
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="relative border rounded-xl overflow-hidden bg-muted/10 aspect-square" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            <img src={images[currentImageIndex] || '/placeholder.svg'} alt={product.title} className="w-full h-full object-contain" />
            
            {/* Desktop Navigation Arrows */}
            {images.length > 1 && <>
                <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-background/80 hover:bg-background rounded-full p-2 shadow-lg transition-all opacity-0 lg:opacity-100 hover:scale-110">
                  <ChevronLeft className="h-6 w-6" />
                </button>
                <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-background/80 hover:bg-background rounded-full p-2 shadow-lg transition-all opacity-0 lg:opacity-100 hover:scale-110">
                  <ChevronRight className="h-6 w-6" />
                </button>
              </>}
            
            {/* Image Counter */}
            {images.length > 1 && <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-background/80 px-3 py-1 rounded-full text-xs font-medium">
                {currentImageIndex + 1} / {images.length}
              </div>}
          </div>
          
          {/* Thumbnails */}
          {images.length > 1 && <div className="flex gap-2 p-4 overflow-x-auto">
              {images.map((image: string, index: number) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`flex-shrink-0 w-16 h-16 lg:w-20 lg:h-20 rounded-lg overflow-hidden border-2 transition-all ${index === currentImageIndex ? 'border-primary ring-2 ring-primary/30' : 'border-border hover:border-muted-foreground/50'}`}>
        ...
        <div className="p-4 lg:p-0 space-y-6">
          ...
          {/* Description */}
          {product.description && <p className="text-muted-foreground text-base leading-relaxed">
              {product.description}
            </p>}

          {/* Key Features Card */}
          {keyFeatures.length > 0 && <Card className="border rounded-xl">
              <CardContent className="p-6">
                <h3 className="font-bold text-lg mb-4">Key Features</h3>
                <ul className="space-y-3">
                  {keyFeatures.map((feature: string, index: number) => <li key={index} className="flex items-start gap-3">
            ...
            <CardContent className="p-6">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <Avatar className="h-12 w-12">
                    ...
                    <AvatarFallback className="text-lg">
                      {product.profiles?.avatar_url ? product.profiles?.display_name?.[0] || 'U' : getRandomAvatarEmoji(product.profiles?.username || 'user')}
                    </AvatarFallback>
                  </Avatar>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {product.profiles?.verified && <Badge variant="outline" className="text-xs bg-green-100 text-green-700 border-green-200">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant={isFollowing(product.seller_id) ? "secondary" : "default"} size="sm" className="flex-1" onClick={handleFollow}>
                    {isFollowing(product.seller_id) ? <><UserMinus className="h-4 w-4 mr-2" />Unfollow</> : <><UserPlus className="h-4 w-4 mr-2" />Follow</>}
                  </Button>
                </div>
          ...
          {/* Reviews */}
          {sellerReviews.length > 0 && <Card className="border rounded-xl">
              <CardContent className="p-6">
                <h3 className="font-semibold text-lg mb-4">Seller Reviews</h3>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {sellerReviews.slice(0, 5).map(review => <ReviewCard key={review.id} review={review} />)}
                </div>
              </CardContent>
            </Card>}
        </div>
      ...
      <ShareDialog isOpen={showShareDialog} onClose={() => setShowShareDialog(false)} title={product.title} url={window.location.href} text={`Check out this ${product.title} for ₹${product.price}`} />
    </div>;
};
